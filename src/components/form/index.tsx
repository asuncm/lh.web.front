...
const components: any = {
  radio: function (props: any) {
    if (props.children && props.children.length > 0) {
      ...
      return (<Radio.Group defaultValue={''} {...props}>
        {
  ...
  rangePicker: function(props: any) {
    return(<DatePicker.RangePicker {...props}></DatePicker.RangePicker>)
  }
}
    ...
    labelAlign={props.labelAlign || 'right'}>
    {
      options.map((item) => {
        if (!item.visible && components[item.type || '']) {
          ...
          return (<Form.Item key={item.key} name={item.key} label={t(`home.${item.key}`)}>
            <Component {...item} models={props.models} />
          </Form.Item>)
        } else {
          return null
        }
